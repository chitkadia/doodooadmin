		...
		while(true) {
			...
			$arrDocumentList = $this->getDocumentDataAsPerPagination();
	 		...
	 		if(!empty($arrDocumentList)) {
	 			...
	 			for($intI = 0; $intI < COUNT($arrDocumentList); $intI++) {

					$id 				= $arrDocumentList[$intI]['id'];
					$user_id 			= $arrDocumentList[$intI]['user_id'];
					$name 				= $arrDocumentList[$intI]['name'];
					$pretty_name 		= $arrDocumentList[$intI]['pretty_name'];
					$password 			= $arrDocumentList[$intI]['password'];
					$folder 			= $arrDocumentList[$intI]['folder'];
					$folder_id 			= $arrDocumentList[$intI]['folder_id'];
					$link 				= $arrDocumentList[$intI]['link'];
					$opened 			= $arrDocumentList[$intI]['opened'];
					$date 				= $arrDocumentList[$intI]['date'];
					$group_id 			= $arrDocumentList[$intI]['group_id'];
					$is_gif_set 		= $arrDocumentList[$intI]['is_gif_set'];
					$s3_bucketname 		= $arrDocumentList[$intI]['s3_bucketname'];
					$is_converted 		= $arrDocumentList[$intI]['is_converted'];
