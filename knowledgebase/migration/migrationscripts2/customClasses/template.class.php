	...
	function migrateTemplates () {
		...
		$this->objLogger->setMessage("Fetched old tempalte data for page - " . $this->currentPage);
 		...
 		if(!empty($arrOldTemplateData)) {
 			...
 			for($intI = 0; $intI < COUNT($arrOldTemplateData); $intI++) {

print "this->counter :: " . $this->counter++ . "\n";

 				$id 						= $arrOldTemplateData[$intI]['id'];
 				$title 						= $arrOldTemplateData[$intI]['title'];
 				$subject 					= $arrOldTemplateData[$intI]['subject'];
 				$content 					= $arrOldTemplateData[$intI]['content'];
 				$categories 				= $arrOldTemplateData[$intI]['categories'];
 				$template_categories_id 	= $arrOldTemplateData[$intI]['template_categories_id'];
 				$template_type_id 			= $arrOldTemplateData[$intI]['template_type_id'];
 				$type 						= $arrOldTemplateData[$intI]['type']; 				
 				$user_id 					= $arrOldTemplateData[$intI]['user_id'];
 				$created_date 				= $arrOldTemplateData[$intI]['created_date'];
 				$updated_date 				= $arrOldTemplateData[$intI]['updated_date'];
 				$total_opened_email 		= $arrOldTemplateData[$intI]['total_opened_email'];
 				$group_id 					= $arrOldTemplateData[$intI]['group_id'];
 				$total_opened_campaign 		= $arrOldTemplateData[$intI]['total_opened_campaign'];
 				$used_with_campaign 		= $arrOldTemplateData[$intI]['used_with_campaign'];
 				$performance 				= $arrOldTemplateData[$intI]['performance'];
 				
 				// not using anywhere
 				$attachment 				= $arrOldTemplateData[$intI]['attachment'];
 				$attachement_url 			= $arrOldTemplateData[$intI]['attachement_url'];
 				$template_id 				= $arrOldTemplateData[$intI]['template_id'];
 				$total_sent_campaign 		= $arrOldTemplateData[$intI]['total_sent_campaign'];
 				$ctr_campaign 				= $arrOldTemplateData[$intI]['ctr_campaign'];
