...
CLASS DOCUMENTLINKS EXTENDS GENERALFUNCTIONS {
	...
	function __construct ($objLogger) {
		...
		$this->linkDomain = SH_DOCUMENT_VIEWER_LINK;
	...
	function startMigrationForDocumentLinkVisit ($arrParams = array()) {
		if(DEBUG) {
			print "startMigrationForDocumentLinkVisit ()";
		}
 		
 		$this->objLogger->setMessage('inside startMigrationForDocumentLinkVisit');

		$documentLinkId = 0;
		if(isset($arrParams['documentLinkId']) && is_numeric($arrParams['documentLinkId']) && $arrParams['documentLinkId'] > 0) {
			$documentLinkId = $arrParams['documentLinkId'];
		}


		$link = '';
		if(isset($arrParams['link']) && !empty($arrParams['link'])) {
			$link = $arrParams['link'];
		}

		if($documentLinkId == 0 || empty($link)) {
			return false;
		}

		while(true) {

			$arrFileLinkLocationInformation = $this->getDataForDocumentLinkvisit($arrParams);

			if(!empty($arrFileLinkLocationInformation)) {
				// foreach ($arrFileLinkLocationInformation as $key => $arrEachViewedFileDataFromOldDB) {
				for($intI = 0; $intI < COUNT($arrFileLinkLocationInformation); $intI++) {
					...
					$id 				= $arrFileLinkLocationInformation[$intI]['id'];
					$file_id 			= $arrFileLinkLocationInformation[$intI]['file_id'];
					$open_datetime 		= $arrFileLinkLocationInformation[$intI]['open_datetime'];
					$view_duration 		= $arrFileLinkLocationInformation[$intI]['view_duration'];
					$filelink_link 		= $arrFileLinkLocationInformation[$intI]['filelink_link'];
					$contact_id 		= $arrFileLinkLocationInformation[$intI]['contact_id'];
					$stamp 				= $arrFileLinkLocationInformation[$intI]['stamp'];
					$slide_number 		= $arrFileLinkLocationInformation[$intI]['slide_number'];
					$ipaddress 			= $arrFileLinkLocationInformation[$intI]['ipaddress'];
					$city 				= $arrFileLinkLocationInformation[$intI]['city'];
					$state 				= $arrFileLinkLocationInformation[$intI]['state'];
					$country 			= $arrFileLinkLocationInformation[$intI]['country'];
					$browser_name 		= $arrFileLinkLocationInformation[$intI]['browser_name'];
					$os_platform 		= $arrFileLinkLocationInformation[$intI]['os_platform'];
					$filelink_id 		= $arrFileLinkLocationInformation[$intI]['filelink_id'];
					$counter 			= $arrFileLinkLocationInformation[$intI]['counter'];
					$type 				= $arrFileLinkLocationInformation[$intI]['type'];
					// $filelinkId 		= $arrFileLinkLocationInformation[$intI]['filelinkId'];


					$filelinkId  		= $documentLinkId;
	...
	function insertIntoDocumentLinkVisitLogs ($arrParams = array()) {
		...
		if(!empty($arrListOfLogs)) {
			...
			$strlinkVisitLogInsert = "";

			for($intI = 0; $intI < COUNT($arrListOfLogs); $intI++) {

				$id 				= 	$arrListOfLogs[$intI]['id'];
				$file_id 			= 	$arrListOfLogs[$intI]['file_id'];
				$open_datetime 		= 	$arrListOfLogs[$intI]['open_datetime'];
				$view_duration 		= 	$arrListOfLogs[$intI]['view_duration'];
				$filelink_link 		= 	$arrListOfLogs[$intI]['filelink_link'];
				$contact_id 		= 	$arrListOfLogs[$intI]['contact_id'];
				$stamp 				= 	$arrListOfLogs[$intI]['stamp'];
				$slide_number 		= 	$arrListOfLogs[$intI]['slide_number'];
				$ipaddress 			= 	$arrListOfLogs[$intI]['ipaddress'];
				$city 				= 	$arrListOfLogs[$intI]['city'];
				$state 				= 	$arrListOfLogs[$intI]['state'];
				$country 			= 	$arrListOfLogs[$intI]['country'];
				$browser_name 		= 	$arrListOfLogs[$intI]['browser_name'];
				$os_platform 		= 	$arrListOfLogs[$intI]['os_platform'];
				$filelink_id 		= 	$arrListOfLogs[$intI]['filelink_id'];
				$counter 			= 	$arrListOfLogs[$intI]['counter'];
				$type 				= 	$arrListOfLogs[$intI]['type'];
	...
	function getDataForDocumentLinkvisit ($arrParams = array()) {
		if(DEBUG) {
		...
		$qrySel = "	SELECT vf.*
					FROM viewed_file vf
					WHERE vf.filelink_link = '" . addslashes($link) . "' ";

		// if($this->lastDocumentLinkVisitId > 0) {
		// 	$qrySel .= " AND vf.id > '" . addslashes($this->lastDocumentLinkVisitId) . "' ";
		// }

		$qrySel .= " 	GROUP BY vf.stamp 
		 				ORDER BY vf.id ";
...
$this->objLogger->setMessage('TOTAL RECORDS FOUND :: ', COUNT($arrDirectGeneratedLink));
			if(!empty($arrDirectGeneratedLink)) {
				...
				for($intI = 0; $intI < COUNT($arrDirectGeneratedLink); $intI++) {

					$id 					= $arrDirectGeneratedLink[$intI]['id'];
					$user_id 				= $arrDirectGeneratedLink[$intI]['user_id'];
					$file_id 				= $arrDirectGeneratedLink[$intI]['file_id'];
					$link 					= $arrDirectGeneratedLink[$intI]['link'];
					$position 				= $arrDirectGeneratedLink[$intI]['position'];
					$contact_id 			= $arrDirectGeneratedLink[$intI]['contact_id'];
					$title 					= $arrDirectGeneratedLink[$intI]['title'];
					$opened_links 			= $arrDirectGeneratedLink[$intI]['opened_links'];
					$opened_date 			= $arrDirectGeneratedLink[$intI]['opened_date'];
					$visitor_info 			= $arrDirectGeneratedLink[$intI]['visitor_info'];
					$allowed_download 		= $arrDirectGeneratedLink[$intI]['allowed_download'];
					$track_forwad 			= $arrDirectGeneratedLink[$intI]['track_forwad'];
					$password 				= $arrDirectGeneratedLink[$intI]['password'];
					$created_date 			= $arrDirectGeneratedLink[$intI]['created_date'];
 					...
 					// if(is_numeric($accountId) && $accountId > 0) {
						...
						$arrFieldsToAsk = array();
 						...
 						if(!$documentLinkId) {
							...
							$this->objLogger->setMessage('starting to import document visits logs.....');

							$arrParamsForInsertingDocumentVistsAndLogs = array();
							$arrParamsForInsertingDocumentVistsAndLogs['documentLinkId'] = $documentLinkId;
							$arrParamsForInsertingDocumentVistsAndLogs['link'] = $link;

							$this->startMigrationForDocumentLinkVisit($arrParamsForInsertingDocumentVistsAndLogs);
	...
	function migrateDocumentLinksNewFlow () {
		...
		do {
			...
			if(!empty($arrFileLinkData)) {
				...
				for($intI = 0; $intI < COUNT($arrFileLinkData); $intI++) {

					$id							=	$arrFileLinkData[$intI]['id'];
					$user_id					=	$arrFileLinkData[$intI]['user_id'];
					$file_id					=	$arrFileLinkData[$intI]['file_id'];
					$link						=	$arrFileLinkData[$intI]['link'];
					$title						=	$arrFileLinkData[$intI]['title'];
					$opened_links				=	$arrFileLinkData[$intI]['opened_links'];
					$created_date				=	$arrFileLinkData[$intI]['created_date'];
					$opened_date				=	$arrFileLinkData[$intI]['opened_date'];
					$is_email					=	$arrFileLinkData[$intI]['is_email'];
					$is_campaign				=	$arrFileLinkData[$intI]['is_campaign'];
					$template_id				=	$arrFileLinkData[$intI]['template_id'];
					$campaign_template_id		=	$arrFileLinkData[$intI]['campaign_template_id'];
					$position					=	$arrFileLinkData[$intI]['position'];
					$visitor_info				=	$arrFileLinkData[$intI]['visitor_info'];
					$allow_download				=	$arrFileLinkData[$intI]['allow_download'];
					$password					=	$arrFileLinkData[$intI]['password'];
					$status						=	$arrFileLinkData[$intI]['status'];
...
print "lastDocumentLinkId TO RETURN  :: " . $lastDocumentLinkId . "\n";
	...
	function insertIntoDocumentLinkFile ($arrParams = array()) {
		...
		if(!empty($arrFileRecordsForLinkCode)) {
			...
			for($intI = 0; $intI < COUNT($arrFileRecordsForLinkCode); $intI++) {

				if(!empty($arrFileRecordsForLinkCode[$intI]['file_id'])) {

					$arrFilesInLink = explode(",", $arrFileRecordsForLinkCode[$intI]['file_id']);
					...
					if(!empty($arrFilesInLink)) {
						...
						for($intJ = 0; $intJ < COUNT($arrFilesInLink); $intJ++) {

							$eachDocumentId = trim($arrFilesInLink[$intJ]);
...
print "this->counterdirectLink :: " . $this->counterdirectLink++ . "\n";
	...
	function insertIntoDocumentLinkTable ($arrParams) {
		...
		$randomDomain = SH_DOCUMENT_VIEWER_LINK;

		$qryIns = "	INSERT INTO document_links (account_id, user_id, source_id, name, short_description, link_domain, link_code, old_link, type, is_set_expiration_date, expires_at, allow_download, password_protected, access_password, ask_visitor_info, visitor_info_payload, snooze_notifications, remind_not_viewed, remind_at, status, created, modified)
