...
class COMMONDBFUNC {
	...
	function COMMONDBFUNC ($host = HOST_NEW, $password = PASSWORD_NEW, $userName = USERNAME_NEW, $dbName = DBNAME_NEW) {
		...
		$this->removeForeignKeyChecks();
	}

	function __destruct() {
		$this->addForeignKeyChecks();
	}
...
print "qrySel :: " . $qrySel . "\n";
		...
		$objDBResult = $this->executeQuery($qrySel);
print "QUERY EXECUTED SUCCESSFULLY...0000";
		if(!$objDBResult) {
			print "Error occur.";
		}
		return;
...
print "QUERY EXECUTED SUCCESSFULLY...11111";
		$objDBResult = $this->executeQuery($qrySel);
