...
if (defined("SH_API_ENDPOINT")) {
    $sh_app->add(function (\Psr\Http\Message\ServerRequestInterface $request, \Psr\Http\Message\ResponseInterface $response, $next) {
        ...
        if (!$request->isOptions()) {
            ...
            if ($route == "/campaigns/preview-csv" || $route == "/campaigns/domain-block/create") {

            } else if ($route == "/documents/create") {
                ...
                if (!empty($token_info)) {
                    ...
                    if (!isset($public_resources[$route])) {
                        ...
                        if ($row["account_status"] == $status_delete) {
                            // Fetch error code & message and return the response
                            return \App\Components\ErrorComponent::outputError($response, "api_messages/ACCOUNT_DELETED");
                        }

                        if ($row["account_status"] != $status_active) {
                            // Fetch error code & message and return the response
                            return \App\Components\ErrorComponent::outputError($response, "api_messages/ACCOUNT_CLOSED");
                        }
                    ...
                    $model_us = new \App\Models\UserSettings();
                    ...
                    $condition_us = [
                        ...
                        "where" => [
                            ["where" => ["user_id", "=", $token_info[0]]],
                            ["where" => ["app_constant_var_id", "=", $user_timezone]]
                        ]
                    ];
                    $timezone_value = $model_us->fetch($condition_us);
                    $time_zone_set = \DEFAULT_TIMEZONE;
                    if (!empty($timezone_value["value"])) {
                        $time_zone_set = $timezone_value["value"];
                    }
